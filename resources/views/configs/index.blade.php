...
@section('content')
    ...
    <ul class="nav nav-pills mb-24 gap-2" id="configTab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active d-flex align-items-center gap-2" id="tab-preset-btn"
                data-bs-toggle="pill" data-bs-target="#tab-preset" type="button">
                <iconify-icon icon="solar:bolt-outline" class="text-lg"></iconify-icon>
                Pilih Preset
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link d-flex align-items-center gap-2" id="tab-kelola-btn"
                data-bs-toggle="pill" data-bs-target="#tab-kelola" type="button">
                <iconify-icon icon="solar:list-check-outline" class="text-lg"></iconify-icon>
                Kelola Preset
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link d-flex align-items-center gap-2" id="tab-manual-btn"
                data-bs-toggle="pill" data-bs-target="#tab-manual" type="button">
            ...
            <div class="card">
                <div class="card-body p-32">
                    <div class="d-flex align-items-center gap-3 mb-24">
                        <div class="w-48-px h-48-px bg-primary-100 text-primary-600 rounded-circle d-flex justify-content-center align-items-center">
                            <iconify-icon icon="solar:bolt-outline" class="text-xl"></iconify-icon>
                        </div>
                        <div>
                            <h5 class="fw-semibold mb-4">Terapkan Preset Cepat</h5>
                            <p class="text-secondary-light text-sm mb-0">Pilih preset sesuai usia tanaman, klik <strong>Terapkan</strong> untuk langsung mengupdate threshold pH, Suhu, dan PPM.</p>
                        </div>
                    </div>
                    ...
                    <div class="border radius-12 p-20 mb-28" style="background:var(--primary-50,#f0fdf4);">
                        ...
                        <div class="row gy-3 text-sm">
                            @php
                                $activeParams = [
                                    ['key'=>'ph',   'label'=>'pH Air',       'icon'=>'fluent:drop-20-filled', 'color'=>'info'],
                                    ['key'=>'suhu', 'label'=>'Suhu Air',     'icon'=>'mdi:thermometer',       'color'=>'warning'],
                                    ['key'=>'ppm',  'label'=>'PPM Nutrisi',  'icon'=>'mdi:flask',             'color'=>'success'],
                                ];
                            @endphp
                            @foreach($activeParams as $p)
                                ...
                                <div class="col-sm-4">
                                    <div class="d-flex align-items-center gap-2">
                                        <div class="w-36-px h-36-px bg-{{ $p['color'] }}-focus text-{{ $p['color'] }}-main rounded-circle d-flex justify-content-center align-items-center flex-shrink-0">
                                ...
                                <div class="col-sm-6 col-xl-3">
                                    <div class="border radius-12 p-20 h-100 d-flex flex-column preset-card"
                                        ...
                                        @if($preset->is_default)
                                            <span class="badge bg-primary-100 text-primary-600 text-xs mb-12 align-self-start">Bawaan</span>
                                        @else
                                            <span class="badge bg-success-focus text-success-main text-xs mb-12 align-self-start">Kustom</span>
                                        @endif
                                            ...
                                            <div class="col-12">
                                                <div class="d-flex justify-content-between py-6 border-bottom">
                                                    <span class="text-secondary-light d-flex align-items-center gap-1">
                                                        <iconify-icon icon="fluent:drop-20-filled" class="text-info-main"></iconify-icon> pH
                                                    </span>
                                                    <strong>{{ number_format($preset->ph_min,1) }} – {{ number_format($preset->ph_max,1) }}</strong>
                                                </div>
                                            </div>
                                            <div class="col-12">
                                                <div class="d-flex justify-content-between py-6 border-bottom">
                                                    <span class="text-secondary-light d-flex align-items-center gap-1">
                                                        <iconify-icon icon="mdi:thermometer" class="text-warning-main"></iconify-icon> Suhu
                                                    </span>
                                                    <strong>{{ number_format($preset->suhu_min,1) }} – {{ number_format($preset->suhu_max,1) }} °C</strong>
                                                </div>
                                            </div>
                                            <div class="col-12">
                                                <div class="d-flex justify-content-between py-6">
                                                    <span class="text-secondary-light d-flex align-items-center gap-1">
                                                        <iconify-icon icon="mdi:flask" class="text-success-main"></iconify-icon> PPM
                                                    </span>
                                                    <strong>{{ (int)$preset->ppm_min }} – {{ (int)$preset->ppm_max }} ppm</strong>
                                                </div>
                                            </div>
                                        </div>

                                        <form action="{{ route('configs.preset.apply') }}" method="POST" class="mt-auto">
                                            @csrf
                                            <input type="hidden" name="preset_id" value="{{ $preset->id }}">
                                            <button type="submit" class="btn btn-primary btn-sm w-100 radius-8">
                                                <iconify-icon icon="solar:bolt-outline" class="me-1"></iconify-icon>
                                                Terapkan
                                            </button>
                                        </form>
                                    </div>
            ...
            <div class="row gy-24">
                ...
                <div class="col-lg-7">
                    <div class="card">
                        <div class="card-body p-32">
                            <div class="d-flex align-items-center gap-3 mb-24">
                                <div class="w-48-px h-48-px bg-success-focus text-success-main rounded-circle d-flex justify-content-center align-items-center">
                                    <iconify-icon icon="solar:list-check-outline" class="text-xl"></iconify-icon>
                                </div>
                                <div>
                                    <h5 class="fw-semibold mb-4">Daftar Preset</h5>
                                    <p class="text-secondary-light text-sm mb-0">{{ $presets->count() }} preset tersimpan. Semua preset dapat dihapus.</p>
                                </div>
                            </div>
                                                ...
                                                <tr>
                                                    <td>
                                                        ...
                                                        @if($preset->description)
                                                            <span class="text-xs text-secondary-light">{{ Str::limit($preset->description, 55) }}</span>
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td class="text-center">
                                                        <div class="d-flex gap-2 justify-content-center">
                                                            <form action="{{ route('configs.preset.apply') }}" method="POST">
                                                                @csrf
                                                                <input type="hidden" name="preset_id" value="{{ $preset->id }}">
                                                                <button type="submit" class="btn btn-primary btn-sm radius-8" title="Terapkan preset ini">
                                                                    <iconify-icon icon="solar:bolt-outline"></iconify-icon>
                                                                </button>
                                                            </form>
                                                            <form action="{{ route('configs.preset.destroy', $preset) }}" method="POST"
                                                                onsubmit="return confirm('Hapus preset « {{ $preset->name }} »?')">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="btn btn-danger-focus btn-sm radius-8" title="Hapus preset">
                                                                    <iconify-icon icon="solar:trash-bin-trash-outline" class="text-danger-main"></iconify-icon>
                                                                </button>
                                                            </form>
                                                        </div>
                                                    </td>
                        ...
                        <div class="card-body p-32">
                            <div class="d-flex align-items-center gap-3 mb-24">
                                ...
                                <div>
                                    ...
                                    <p class="text-secondary-light text-sm mb-0">Preset hanya mencakup pH, Suhu Air, dan PPM Nutrisi.</p>
                                </div>
                            </div>
                            ...
                            <form id="formAddPreset" action="{{ route('configs.preset.store') }}" method="POST"
                                ...
                                <div class="mb-16">
                                    ...
                                    @error('name')<span class="text-danger-main text-xs">{{ $message }}</span>@enderror
                                </div>
                                    ...
                                    <div class="row gy-2">
                                        <div class="col-6">
                                            ...
                                            <input type="text" inputmode="decimal" name="ph_min"
                                                ...
                                                data-min="0" data-max="14" required>
                                            @error('ph_min')<span class="text-danger-main text-xs">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-6">
                                            ...
                                            <input type="text" inputmode="decimal" name="ph_max"
                                                ...
                                                data-min="0" data-max="14" required>
                                            @error('ph_max')<span class="text-danger-main text-xs">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="border radius-8 p-16 mb-12">
                                    ...
                                    <div class="row gy-2">
                                        <div class="col-6">
                                            ...
                                            @error('suhu_min')<span class="text-danger-main text-xs">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-6">
                                            <label class="form-label text-xs text-secondary-light mb-4">Max</label>
                                            <div class="input-group">
                                            ...
                                            @error('suhu_max')<span class="text-danger-main text-xs">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="border radius-8 p-16 mb-20">
                                    ...
                                    <div class="row gy-2">
                                        <div class="col-6">
                                            ...
                                            @error('ppm_min')<span class="text-danger-main text-xs">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-6">
                                            <label class="form-label text-xs text-secondary-light mb-4">Max</label>
                                            <div class="input-group">
                                            ...
                                            @error('ppm_max')<span class="text-danger-main text-xs">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="card-body p-32">
                            <div class="d-flex align-items-center gap-3 mb-32">
                                ...
                                <div>
                                    ...
                                    <p class="text-secondary-light text-sm mb-0">Edit langsung semua parameter termasuk Ketinggian Air Tandon.</p>
                                </div>
                            </div>
                            ...
                            <form action="{{ route('configs.update') }}" method="POST" class="needs-decimal-fix">
                                ...
                                @php
                                    $manualParams = [
                                        ['key'=>'ph',             'label'=>'pH Air',                'icon'=>'fluent:drop-20-filled', 'color'=>'info',    'unit'=>'',   'desc'=>'Tingkat keasaman larutan nutrisi (0–14)',          'step'=>'0.1',  'decimal'=>true],
                                        ['key'=>'suhu',           'label'=>'Suhu Air',              'icon'=>'mdi:thermometer',       'color'=>'warning', 'unit'=>'°C', 'desc'=>'Suhu larutan nutrisi',                            'step'=>'0.5',  'decimal'=>true],
                                        ['key'=>'ppm',            'label'=>'PPM Nutrisi',           'icon'=>'mdi:flask',             'color'=>'success', 'unit'=>'ppm','desc'=>'Konsentrasi larutan nutrisi',                     'step'=>'10',   'decimal'=>false],
                                        ['key'=>'ketinggian_air', 'label'=>'Ketinggian Air Tandon', 'icon'=>'mdi:water-pump',        'color'=>'primary', 'unit'=>'cm', 'desc'=>'Batas ketinggian air tandon (kontrol pompa isi)',  'step'=>'0.5',  'decimal'=>true],
                                    ];
                                @endphp
                                        ...
                                        <div class="d-flex align-items-center gap-3 mb-20">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-secondary-light mb-0">{{ $param['desc'] }}
                                                    @if($cfg && $cfg->unit) · Satuan: <strong>{{ $cfg->unit }}</strong> @endif
                                                </p>
                                            </div>
                                        </div>
                                        <div class="row gy-3">
                                            <div class="col-sm-6">
                                                <label class="form-label text-sm fw-medium text-secondary-light text-uppercase">Batas Minimum</label>
                                                <div class="input-group">
                                                    <input type="text" inputmode="{{ $param['decimal'] ? 'decimal' : 'numeric' }}"
                                                        name="{{ $param['key'] }}_min"
                                                    ...
                                                    @if($param['unit'])<span class="input-group-text text-secondary-light">{{ $param['unit'] }}</span>@endif
                                                </div>
                                                @error($param['key'].'_min')<span class="text-danger-main text-xs d-block mt-4">{{ $message }}</span>@enderror
                                            </div>
                                            <div class="col-sm-6">
                                                <label class="form-label text-sm fw-medium text-secondary-light text-uppercase">Batas Maksimum</label>
                                                <div class="input-group">
                                                    <input type="text" inputmode="{{ $param['decimal'] ? 'decimal' : 'numeric' }}"
                                                        name="{{ $param['key'] }}_max"
                                                ...
                                                @error($param['key'].'_max')<span class="text-danger-main text-xs d-block mt-4">{{ $message }}</span>@enderror
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
...
document.querySelectorAll('.decimal-input').forEach(function (inp) {
    inp.addEventListener('input', function () { normalizeDecimal(this); });
...
document.querySelectorAll('.integer-input').forEach(function (inp) {
    inp.addEventListener('input', function () { normalizeInteger(this); });
});
